...
class TORA:
    def __init__(self, trajectory_discretisation=15, trajectory_duration=1.0):
        """
        ...
        self.trajectory_num_knots = int(trajectory_discretisation * trajectory_duration) + 1  # in units (number of knots)
        ...
        ipopt_options["max_cpu_time"] = 1.0  # in seconds
        ...
        ipopt_options["acceptable_constr_viol_tol"] = 0.05  # default: 0.01
    ...
    def slerp(self, q1_np, q2_np, t):
        """Spherical Linear Interpolation (SLERP) for quaternions."""
        q1 = R.from_quat(q1_np)
        q2 = R.from_quat(q2_np)
        times = [0, 1]
        rots = R.from_quat([q1.as_quat(), q2.as_quat()])
        slerp = Slerp(times, rots)

        # Interpolate at t
        interp_rot = slerp(t)
        q_interp = interp_rot.as_quat()

        # if np.dot(q_interp, q1_np) < 0:
        #     q_interp = -q_interp
        
        # Ensure the quaternions are in the correct format (w, x, y, z)
        if q_interp[-1] < 0:
            q_interp = -q_interp
    ...
    def plan(self,
        ...
        # if np.dot(current_ee_quat, target_ee_quat) < 0:
        #     target_ee_quat = -target_ee_quat
        # Ensure the quaternions are in the correct format (w, x, y, z)
        if current_ee_quat[-1] < 0:
            current_ee_quat = -current_ee_quat
        if target_ee_quat[-1] < 0:
            target_ee_quat = -target_ee_quat
        ...
        for k in range(1, jl_problem.num_knots + 1):
        ...
        print('current', f"{current_ee_trans=} {current_ee_quat=}")
        print('target', f"{target_ee_trans=} {target_ee_quat=}")
        print('py_eff_k', f"{py_eff_pos_k=} {py_eff_quat_k=}")
