...
class DeviceConnection:
    ...
    @staticmethod
    def createTcpConnection(ip, user='admin', passw='admin'): 
        """
        returns RouterClient required to create services and send requests to device or sub-devices,
        """

        return DeviceConnection(ip, port=DeviceConnection.TCP_PORT, credentials=(user, passw))

    @staticmethod
    def createUdpConnection(ip, user='admin', passw='admin'): 
        """        
        returns RouterClient that allows to create services and send requests to a device or its sub-devices @ 1khz.
        """

        return DeviceConnection(ip, port=DeviceConnection.UDP_PORT, credentials=(user, passw))
